...
export const AdminDashboard: React.FC = () => {
  ...
  const ratingFilter = useSelector((state: RootState) => state.admin.ratingFilter);
  const mediaFilter = useSelector((state: RootState) => state.admin.mediaFilter);
  const statusFilter = useSelector((state: RootState) => state.admin.statusFilter);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) => {
      ...
      // Rating
      const matchesRating = ratingFilter === 'ALL' || r.overallRating === ratingFilter;
  ...
  const grievanceCount = records.filter((r) => r.isGrievance || r.overallRating === 'Could Be Better').length;
      ...
      <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-2 sm:p-6 overflow-y-auto">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <div className="bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-2xl text-xs flex items-center gap-2">
                <Icon icon="ph:user-circle-bold" className="w-5 h-5 text-amber-600" />
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase">Authenticated Admin</div>
                  <div className="font-extrabold text-slate-900">{adminEmail}</div>
                </div>
              </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto border-b border-slate-200 py-3 no-scrollbar shrink-0">
            {[
              { id: 'analytics', label: 'Analytics & KPI Overview', icon: 'ph:chart-bar-bold' },
              { id: 'feedbacks', label: `All Patient Feedbacks (${records.length})`, icon: 'ph:table-bold' },
              { id: 'grievances', label: `Grievances & Escalations (${grievanceCount})`, icon: 'ph:warning-circle-bold' },
              { id: 'facilities', label: 'District Facility Metrics', icon: 'ph:hospital-bold' },
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          <div className="flex-1 overflow-y-auto pt-4 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-slate-400 uppercase">Total Feedbacks</div>
                <div className="text-2xl font-black text-slate-900 mt-0.5">{totalCount}</div>
                <div className="text-[10px] font-bold text-emerald-600 mt-0.5">↑ +14% vs yesterday</div>
              </div>

              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-slate-400 uppercase">Avg Rating</div>
                <div className="text-2xl font-black text-amber-600 mt-0.5 flex items-center gap-1">
                  <span>4.8</span>
                  <span className="text-xs text-slate-400">/ 5.0</span>
                </div>
                <div className="text-[10px] font-bold text-amber-600 mt-0.5">94.2% Satisfaction</div>
              </div>
              ...
              <div className="bg-amber-50/60 border border-amber-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-amber-800 uppercase flex items-center justify-between">
                  ...
                  <Icon icon="ph:microphone-fill" className="w-3.5 h-3.5 text-amber-600" />
                </div>
                <div className="text-2xl font-black text-amber-900 mt-0.5">{audioNotesCount} Logs</div>
                <div className="text-[10px] font-bold text-amber-700 mt-0.5">Playable Audio Files</div>
              </div>
              ...
              <div className="bg-blue-50/60 border border-blue-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-blue-800 uppercase flex items-center justify-between">
                  ...
                  <Icon icon="ph:camera-bold" className="w-3.5 h-3.5 text-blue-600" />
                </div>
                <div className="text-2xl font-black text-blue-900 mt-0.5">{imagesCount} Photos</div>
                <div className="text-[10px] font-bold text-blue-700 mt-0.5">Image Evidence Logs</div>
              </div>
              ...
              <div className="bg-red-50/60 border border-red-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-red-800 uppercase flex items-center justify-between">
                  ...
                  <Icon icon="ph:warning-bold" className="w-3.5 h-3.5 text-red-600" />
                </div>
                <div className="text-2xl font-black text-red-900 mt-0.5">{grievanceCount} Cases</div>
                <div className="text-[10px] font-bold text-red-700 mt-0.5">High Priority SLA</div>
              </div>

              <div className="bg-emerald-50/60 border border-emerald-200 rounded-2xl p-3.5 shadow-sm">
                <div className="text-[10px] font-extrabold text-emerald-800 uppercase">SLA Resolved</div>
                <div className="text-2xl font-black text-emerald-900 mt-0.5">{resolvedCount} Cases</div>
                <div className="text-[10px] font-bold text-emerald-700 mt-0.5">96.4% Compliance</div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        <Icon icon="ph:chart-pie-slice-bold" className="w-4 h-4 text-amber-600" />
                        Patient Satisfaction Index by Category
                      </span>
                      <span className="text-[11px] text-slate-400 font-bold">Target: &gt;90%</span>
                    </h3>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        <Icon icon="ph:waveform-bold" className="w-4 h-4 text-amber-600" />
                        Feedback Submission Format Telemetry
                      </span>
                      <span className="text-[11px] text-slate-400 font-bold">Multimodal AI Logs</span>
                    </h3>

                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div className="bg-white border border-slate-200 p-3 rounded-xl">
                        <Icon icon="ph:microphone-fill" className="w-6 h-6 text-amber-600 mx-auto mb-1" />
                        <div className="text-lg font-black text-slate-900">42%</div>
                        <div className="text-[10px] font-bold text-slate-500">Voice Recording</div>
                      </div>

                      <div className="bg-white border border-slate-200 p-3 rounded-xl">
                        <Icon icon="ph:text-t-bold" className="w-6 h-6 text-blue-600 mx-auto mb-1" />
                        <div className="text-lg font-black text-slate-900">38%</div>
                        <div className="text-[10px] font-bold text-slate-500">Typed Text</div>
                      </div>

                      <div className="bg-white border border-slate-200 p-3 rounded-xl">
                        <Icon icon="ph:camera-bold" className="w-6 h-6 text-emerald-600 mx-auto mb-1" />
                        <div className="text-lg font-black text-slate-900">20%</div>
                        <div className="text-[10px] font-bold text-slate-500">Photo Attachments</div>
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-900 flex items-center gap-2 font-semibold">
                      <Icon icon="ph:info-bold" className="w-4 h-4 text-amber-600 shrink-0" />
                      <span>Voice recordings automatically transcribed & indexed for CMO rapid audit.</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
              <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                ...
                <div className="relative w-full md:w-96">
                  ...
                  />
                  <Icon icon="ph:magnifying-glass-bold" className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                  {searchQuery && (
                    <button
                ...
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                  ...
                  <select
                    value={ratingFilter}
                    onChange={(e) => dispatch(setRatingFilter(e.target.value as any))}
                  ...
                  <select
                    value={mediaFilter}
                    onChange={(e) => dispatch(setMediaFilter(e.target.value as any))}
                    className="bg-white border border-slate-300 rounded-xl py-2 px-3 text-xs font-bold text-slate-700 cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-500/30"
                  >
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              <div className="px-4 py-3 bg-slate-900 text-white flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400 font-medium hidden sm:inline">
                  Click any row to inspect complete feedback history & play voice recordings
                </span>
              </div>

              {filteredRecords.length === 0 ? (
                <div className="p-12 text-center text-slate-400">
                  <Icon icon="ph:tray-bold" className="w-12 h-12 mx-auto mb-2 text-slate-300" />
                  <p className="text-sm font-bold text-slate-600">No matching feedback records found.</p>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-2">
                                <span className="font-mono font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200 text-[11px]">
                                  {record.trackingId}
                                </span>
                                {record.urgency === 'High SLA Priority' && (
                                  <span className="w-2 h-2 rounded-full bg-red-600 animate-pulse" title="High SLA Priority" />
                                )}
                              </div>
                            ...
                            <td className="py-3.5 px-4 max-w-xs">
                              <div className="font-semibold text-slate-800 truncate" title={record.facilityName}>
                                {record.facilityName}
                              </div>
                              <div className="text-[11px] text-slate-400 font-medium">
                                {record.district}
                            ...
                            {/* Rating */}
                            <td className="py-3.5 px-4">
                              <span className={`px-2.5 py-1 rounded-full font-extrabold text-[11px] border inline-flex items-center gap-1 ${
                            ...
                            <td className="py-3.5 px-4" onClick={(e) => e.stopPropagation()}>
                              <div className="flex items-center gap-2 flex-wrap">
                                {hasAudio && sampleAudio && (
                                  <AudioPlayerWidget audioUrl={sampleAudio} compact />
                                )}
                                {hasImage && (
                                  <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 font-bold text-[10px] border border-blue-200 inline-flex items-center gap-1">
                                    <Icon icon="ph:camera-bold" className="w-3 h-3 text-blue-600" />
                                    <span>Photo</span>
                                  </span>
                                )}
                                {!hasAudio && !hasImage && (
                                  <span className="text-[11px] text-slate-400 font-medium">Text only</span>
                                )}
                              </div>
                            </td>
                            ...
                            {/* Date & Status */}
                            <td className="py-3.5 px-4">
                              <div className="text-[11px] font-semibold text-slate-500">{record.timestamp}</div>
                              <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full font-extrabold text-[10px] border ${
                            ...
                            <td className="py-3.5 px-4 text-right">
                              ...
                              >
                                <span>Inspect</span>
                                <Icon icon="ph:caret-right-bold" className="w-3 h-3 text-amber-400" />
                              </button>
                            </td>
                          </tr>
                        );
